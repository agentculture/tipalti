...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope tipalti
...
scope:** the default personal scope is **private** (`--scope tipalti
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `tipalti`). Private records are served only to a recall in the same
