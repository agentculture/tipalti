...
name: communicate
description: >
  Cross-repo + mesh communication from tipalti: file tracked GitHub issues
  ...
  the next step lives outside tipalti (a brief for a sibling-repo agent, a
...
Cross-repo alignment across the AgentCulture mesh surfaces in four
distinct channels:
...
The receiving agent must not need tipalti-side context to act. Inline
the relevant content; do not say "see tipalti's plan."

A brief that says "see tipalti#NN" is a bug. The receiving agent will
look at it, get lost in tipalti-specific context that's irrelevant to
...
Tipalti is **not** a registered mesh agent today. The script works
once tipalti has been registered and started via `culture agent
register` + `culture start spark-tipalti`; until then, calling it will
fail with whatever error the Culture CLI returns, which is the right
...
- Post a brief that says "see tipalti's plan" without inlining the
